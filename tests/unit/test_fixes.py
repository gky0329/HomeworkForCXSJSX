    ...
    try:
        ...
        with patch("app.core.engine.error_store.log_activity"), patch("app.core.engine.error_store.add_knowledge_point"):
            engine._on_trace_ready(ExecutionResult(
                trace=trace,
                diagnostics="Native debugger: LLDB / DWARF",
            ))

        assert "Native debugger: LLDB / DWARF" in window.statusBar().currentMessage()
    finally:
        window.close()
...
def test_minimal_light_palette_uses_light_surfaces():
    """The white minimal theme must also update inline-style color tokens."""
    ...
    light_qss = stylesheet_for_theme("minimal_light")
    ...
    assert "url(" not in light_qss


def test_minecraft_theme_keeps_original_palette():
    """Minecraft QSS should not inherit the currently active minimal palette."""
    from app.ui.theme.colors import palette_for_theme
    from app.ui.theme.styles import stylesheet_for_theme

    mc = palette_for_theme("mc")
    light = palette_for_theme("minimal_light")
    mc_qss = stylesheet_for_theme("mc")
    ...
    assert "#111611" in mc_qss
    assert "#78A84A" in mc_qss
...
def test_native_debug_smoke_forwards_stdin_to_debug_executor():
    ...
    with patch("tools.native_debug_smoke.DebugExecutor", FakeDebugExecutor):
        result = _run_case(CASES["stdin_sum"], "lldb-dwarf", render=False, config_path=config_path)

    assert result["ok"] is True
    assert captured["backend"] == "lldb-dwarf"
...
def test_native_debug_smoke_forwards_config_to_backend_status():
    """Backend listing should honor a custom config path."""
    from pathlib import Path
    from types import SimpleNamespace
