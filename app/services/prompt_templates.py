...
7. 每道选择题必须有且只有一个正确答案，且正确答案必须出现在 options 中。
8. options 必须写完整选项文本，禁止只写 "A"、"B"、"C"、"D" 或把选项标签当成答案内容。
9. answer 必须是 0-3 的整数索引；禁止输出 "A/B/C/D"、"1/2/3/4" 或答案文本。
10. 出题时先确定一条正确结论，再将这条正确结论改写为 options[answer]；其余三个选项必须是明确错误的常见误解。
11. 正确选项不能是"前半句正确、后半句错误"的复合句；含有"且/并且/必须/只能/不能"的选项要逐个分句核验。
12. 输出前逐题自检：如果 options[answer] 不能独立、完整、正确地回答 question，或者四个选项里没有正确答案，必须重写该题。
